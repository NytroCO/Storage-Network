...
public abstract class ContainerFastNetworkCrafter extends ContainerFastBench implements IStorageContainer {
    ...
    ContainerFastNetworkCrafter(EntityPlayer player, World world, BlockPos pos) {
    ...
    private static void tryRestockGridEntirely(InventoryCrafting matrix, SlotCraftingNetwork slot, IRecipe recipe, ItemStack[] requests) {
        ...
        if (!simple) {
            for (int i = 0; i < 9; i++) {
                if (matrix.getStackInSlot(i).isEmpty()) {
                    ...
                    if (!cached.isEmpty())
                        matrix.stackList.set(i, slot.getTileMaster().request(new ItemStackMatcher(cached, true, false, cached.hasTagCompound()), two ? 1 : cached.getMaxStackSize(), false));
                }
            }
            return;
        }
        ...
        for (int i = 0; i < 9; i++) {
            if (matrix.getStackInSlot(i).isEmpty()) {
                ItemStack cached = requests[i];
                if (!cached.isEmpty())
                    requested[i] = slot.getTileMaster().request(new ItemStackMatcher(cached, true, false, cached.hasTagCompound()), cached.getMaxStackSize(), false);
                matrixFull = false;
            }
        }
    ...
    void bindPlayerInvo(final InventoryPlayer playerInv) {
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
    ...
    final class SlotCraftingNetwork extends SlotCraftingSucks {

        TileMaster tileMaster;

        SlotCraftingNetwork(EntityPlayer player, InventoryCrafting matrix, InventoryCraftResult result, int index, int x, int y) {
        ...
        void setTileMaster(TileMaster tileMaster) {
