...
public abstract class ContainerNetworkBase extends Container implements IStorageContainer {
    ...
    boolean test = false;    @Override
    public InventoryCrafting getCraftMatrix() {
        return this.matrix;
    }

    public abstract void bindHotbar();    @Override
     ...
     * @param tile
     */
    private void craftShift(EntityPlayer player, TileMaster tile) {
        if (matrix == null) {
            return;
        }
        ...
        while (crafted + sizePerCraft <= res.getMaxStackSize()) {
            ...
            //stop if empty
            if (!recipeCurrent.matches(matrix, player.world)) {
